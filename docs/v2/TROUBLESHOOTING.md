...
```
Ask Claude: "What's the system status?"
```
...
### 1. System Not Responding to MCP Tool Calls

**Symptoms:** Claude says it can't connect to the PM Intelligence tools, or tool calls time out.
...
| MCP not configured in Claude | Check `.cursor/mcp.json` configuration |
...
**Symptoms:** Entity resolution, document parsing, or GraphRAG indexer returns errors or times out.
...
| Cause | Fix |
|-------|-----|
| Python service not started | `cd python/entity_resolution && uvicorn main:app --port 5001` |
...
**Investigation:**
```
Ask Claude: "What are the entity resolution stats?"
Ask Claude: "Show me entity reviews"
```
...
**Investigation:**
```
Ask Claude: "Are there any failed processing items?"  (checks DLQ)
```
...
**Symptoms:** MCP tool responses take >5 seconds.
...
2. **Check DLQ:** Ask Claude "What's in the dead letter queue?"
