...
/**
 * Analyze all weekly status update thread replies for wins
 * Uses improved criteria focusing on customer names and quantifiable impact
 */

import { writeFileSync } from 'fs';
import { join } from 'path';

// All thread replies fetched (excluding bot parent messages)
const ALL_THREAD_REPLIES = [
  // Thread 1769052603.990089 (2026-01-22) - Already have this
  {
    "user": "U0900H3NUUT",
...
function extractCustomerName(text: string): string | undefined {
  ...
  const customerPatterns = [
    /(?:with|for|to|at)\s+(NFCU|UBS|Deutshe\s+bank|Deutsche\s+Bank|Lumen|Burnswick|Micron|Travel\s+Port|South\s+African\s+Police\s+Service|MoD|LPL|Jet2|Adobe\s+Consulting)/i,
    /(NFCU|UBS|Deutshe\s+bank|Deutsche\s+Bank|Lumen|Burnswick|Micron|Travel\s+Port|South\s+African\s+Police\s+Service|MoD|LPL|Jet2|Adobe\s+Consulting)\s+(?:customer|client|prospect|enrollment|evaluation|modernization|implementation|demo|conversation)/i,
    /(?:unblocked|pitched|discussed|helped|providing\s+guidance\s+to)\s+(?:Adobe\s+Consulting|UBS|NFCU|Deutshe\s+bank|Deutsche\s+Bank|Lumen|Burnswick|Micron|Travel\s+Port|South\s+African\s+Police\s+Service|MoD|LPL|Jet2)/i,
    /(?:enrollments?|evaluation|modernization|implementation)\s+(?:for|with)\s+(Lumen|Burnswick|Micron|Travel\s+Port|South\s+African\s+Police\s+Service)/i,
  ];
...
function extractQuantifiableMetrics(text: string): { metrics: string; hasRealNumbers: boolean } {
  const realMetricPatterns = [
    ...
    /(\d+)\s*(?:Setup\s+Guide|Troubleshooting|articles?|pdfs?|guides?|features?)\s*(?:published|created|authored|updated|completed)/gi,
    ...
    /first\s+(?:in|across|team)/gi, // "first in AEM" is a quantifiable achievement
  ];
...
function generateSummary(wins: WeeklyWin[]): string {
  ...
  let summary = `\n${'='.repeat(80)}\n`;
  summary += `📈 WEEKLY STATUS UPDATE THREADS - WINS ANALYSIS\n`;
  summary += `📋 Analyzed ${ALL_THREAD_REPLIES.length} thread replies from weekly status updates\n`;
  summary += `${'='.repeat(80)}\n\n`;
...
async function analyzeAllThreadReplies() {
  console.log(`\n📊 Analyzing All Weekly Status Update Thread Replies\n`);
  ...
  try {
    console.log(`📥 Analyzing ${ALL_THREAD_REPLIES.length} thread replies...\n`);
    ...
    const outputFile = join(process.cwd(), `weekly_status_threads_wins_analysis.txt`);
...
export { analyzeAllThreadReplies };
